      ...
      {dots && (
        ...
        >
          {Array.from({ length: totalSlides }, (_, index) => (
            // <div key={index} className="border rounded-full h-3 w-3">
            //   <button
            //     style={{
            //       backgroundColor:
            ...
            <div
              key={index}
              className={`relative  rounded-full h-2 w-2 flex justify-center items-center ${
                index === currentSlide
                  ? "before:absolute before:inset-0 before:rounded-full before:border-2 before:border-current before:animate-ping"
                  : ""
              }`}
            >
              <button
